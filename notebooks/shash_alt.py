...
class SHASHLogScore(LogScore):
    ...
    def d_score(self, Y):
        ...
        #print("x sinh(r): ", np.min(np.sinh(r)), np.max(np.sinh(r)))
        v = 1./(z*z + 1.)
        u = self.tau * w * np.sqrt(v)

        D = np.zeros((self.scale.shape[0], 4))
        D[:, 0] = (u - z*v)/self.scale
        D[:, 1] = z*u + v
        D[:, 2] = w
...
class SHASHo:
    ...
    def logpdf(self, Y):
        ...
        #w = self.tau * np.log(z + np.sqrt(1 + z*z)) - self.nu
        #print("pars1:", np.max(self.loc), np.max(self.scale), np.max(self.nu), np.max(self.tau))
        #print("pars2:", np.min(self.loc), np.min(self.scale), np.min(self.nu), np.min(self.tau))
        #print("w: ", np.min(w), np.max(w))
        s = np.sinh(w)
        #s = 0.5*(np.exp(w) - np.exp(-w))
        #u = (z + np.sqrt(1 + z*z))**self.tau/np.exp(self.nu)
        #s = 0.5*(u - 1/u)
